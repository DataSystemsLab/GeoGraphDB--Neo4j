...
public class GeoReach_Integrate implements ReachabilityQuerySolver
{
	...
	public GeoReach_Integrate(MyRectangle rect, int p_split_pieces, int p_ratio, String p_suffix)
	{	
		ratio = p_ratio;
...
//			long offset = ratio / 20 * node_count;
			try
			{
				...
				while((tempString = reader.readLine())!=null)
				{
					...
					inserter.setNodeProperty(id + offset, "Bitmap_"+split_pieces+suffix+"_"+ratio, bitmap);
				}
				reader.close();
			}
			catch(IOException e)
...
//			long offset = ratio / 20 * node_count;
			try
			{
				...
				while((tempString = reader.readLine())!=null)
				{
					...
					inserter.setNodeProperty(id + offset, "MultilevelBitmap_"+split_pieces+"_"+merge_count+suffix+"_"+ratio, bitmap);
				}
				reader.close();
			}
			catch(Exception e)
...
//		int ratio = 20;
	...
	private boolean TraversalQuery_FullGrids(int start_id, MyRectangle rect, int ratio, int lb_x, int lb_y, int rt_x, int rt_y)
	{
		...
		for(int i = 0;i<jsonArr.size();i++)
		{
			...
			VisitedVertices.add(id);
			judge_time += System.currentTimeMillis() - start;
			boolean reachable = TraversalQuery_FullGrids(id, rect, ratio, lb_x, lb_y, rt_x, rt_y);
