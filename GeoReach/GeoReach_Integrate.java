...
public class GeoReach_Integrate implements ReachabilityQuerySolver
{
	...
	public void LoadCompresedBitmap(int split_pieces, String datasource)
	{
		...
		try
		{
			...
			while((tempString = reader.readLine())!=null)
			{
				...
				inserter.setNodeProperty(id, "Bitmap_"+split_pieces, bitmap);
			}
			reader.close();
		}
		catch(IOException e)
...
//			long offset = ratio / 20 * node_count;
			...
			{
				inserter = BatchInserters.inserter(new File(db_path).getAbsolutePath(),config);
				file = new File("/home/yuhansun/Documents/Real_data/" + datasource + "/GeoReachGrid_"+split_pieces+"/"+filesuffix+"/Bitmap_"+ratio+"_multilevelfull_"+merge_count+".txt");
				reader = new BufferedReader(new FileReader(file));
				reader.readLine();
				tempString = null;
				while((tempString = reader.readLine())!=null)
				{
					...
					inserter.setNodeProperty(id + offset, "MultilevelBitmap_"+split_pieces+"_"+merge_count, bitmap);
				}
				reader.close();
			}
			catch(Exception e)
...
//		int ratio = 20;
	...
	{
		String bitmap_name = "Bitmap_"+split_pieces;
	...
	{	
		String bitmap_name = String.format("MultilevelBitmap_%d_%d%s_%d", split_pieces);
	...
	{
		String bitmap_name = String.format("MultilevelBitmap_%d_%d%s_%d", split_pieces, merge_ratio);
		VisitedVertices.clear();
		long start = System.currentTimeMillis();
		String query = String.format("match (n) where id(n) = %d return n.%s", start_id, bitmap_name);
		String result = Neo4j_Graph_Store.Execute(resource, query);
	...
	{		
		String hasbitmap_name = "HasBitmap_"+split_pieces+"_"+200;
		String bitmap_name = "Bitmap_"+split_pieces;
		long start = System.currentTimeMillis();
